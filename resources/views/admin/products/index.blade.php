...
@section('content')
    <div class="admin-page-header">
        <div>
            ...
            <p class="admin-page-subtitle">Kelola produk UMKM di seluruh marketplace desa</p>
        </div>
    ...
    <div class="admin-card">
        <form method="GET" action="{{ route('admin.products.index') }}" class="admin-toolbar" id="product-filter-form">
            <div class="admin-search">
                ...
                <input type="text" name="search" id="product-search" value="{{ request('search') }}" placeholder="Cari produk..." class="admin-search__input">
            </div>

            <select name="category" id="product-category-filter" class="admin-select">
            ...
            <select name="status" id="product-status-filter" class="admin-select">
                ...
                <tbody>
                    @forelse ($products as $index => $product)
                        ...
                        <tr>
                            ...
                            <td>{{ $product->sellerProfile->business_name }}</td>
                            ...
                            <td>
                                <div class="admin-table__actions">
                                    <button type="button" class="admin-icon-btn" data-modal-target="view-product-{{ $product->id }}" aria-label="Lihat">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg>
                                    </button>
                                    <button type="button" class="admin-icon-btn" data-modal-target="edit-product-{{ $product->id }}" aria-label="Edit">
                                    ...
                                    <button type="button" class="admin-icon-btn admin-icon-btn--danger" data-modal-target="delete-product-{{ $product->id }}" aria-label="Hapus">
                    ...
                    @empty
                        <tr><td colspan="8" class="admin-table__empty">Belum ada produk.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    @foreach ($products as $product)
        <x-modal id="view-product-{{ $product->id }}" title="Detail Produk" size="lg">
            ...
            <div class="admin-modal__meta">
                ...
                <span>•</span>
                <span>{{ $product->sellerProfile->business_name }}</span>
                <span>•</span>
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <x-modal id="edit-product-{{ $product->id }}" title="Edit Produk" size="lg">
                <div class="admin-form-row">
                    ...
                    <div class="admin-form-group">
                        <label>Penjual</label>
                        <select name="seller_profile_id" class="admin-input" required>
                            @foreach ($sellers as $seller)
                                <option value="{{ $seller->id }}" @selected($product->seller_profile_id == $seller->id)>{{ $seller->business_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                        ...
                        <div class="admin-image-manage">
                            @foreach ($product->images as $img)
                                <div class="admin-image-manage__item">
                                    <img src="{{ asset('storage/' . $img->image_url) }}" alt="Galeri produk">
                                    <button type="submit" form="delete-image-form-{{ $img->id }}" class="admin-image-manage__remove" aria-label="Hapus gambar">×</button>
                                </div>
                            @endforeach
                        </div>
                    </div>
        ...
        {{-- Form terpisah buat delete gambar satuan (di luar form edit, biar gak nested <form>) --}}
        @foreach ($product->images as $img)
            <form id="delete-image-form-{{ $img->id }}" method="POST" action="{{ route('admin.products.images.destroy', [$product, $img]) }}">
                @csrf
                @method('DELETE')
            </form>
        @endforeach
    ...
    @push('scripts')
        <script>
            ...
            productSearch?.addEventListener('input', () => {
                ...
                productDebounce = setTimeout(() => productForm.submit(), 400);
            });

            document.getElementById('product-category-filter')?.addEventListener('change', () => productForm.submit());
            document.getElementById('product-status-filter')?.addEventListener('change', () => productForm.submit());
        </script>
    @endpush
@endsection
